    ...
    while n >= 1:
        ...
        for row in table:
            if add[0] == row[0] and correct_number == True:
                postgre_query = """UPDATE phonebook SET phone_number = %s WHERE name = %s """
        ...
        if flag_mult and correct_number == True:
            ...
            postgre_query = """INSERT INTO phonebook(name, phone_number) VALUES (%s, %s) """
...
def fetch_users():
    ...
    if n == 'all':
        ...
        for row in table:
            i += 1
            print("{0:1}{1:4}{2:15}{3:23}{4:5}{5:35}".format(" ", i, " ", row[1], " ", row[2]))
    else:
        print(
            "ID    |             Names           |        Number\n-------------------------------------------------------------------")
        ...
        for row in table:
            i += 1
            print("{0:1}{1:3}{2:15}{3:23}{4:3}{5:35}".format(" ", i, " ", row[1], " ", row[2]))
...
def delete_user():
    ...
    name = input()
    postgre_query = """DELETE FROM phonebook WHERE name = %s """
...
try:
    cond = False
    conn = psycopg2.connect("dbname=ayazhan user=postgres password=ayakbtu")
    cursor = conn.cursor()
